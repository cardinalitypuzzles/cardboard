...
class GoogleInfo(models.Model):
    ...
    autogenerated_sheets_folder_id = models.CharField(
        max_length=44, editable=False, blank=True
    )
    human_files_folder_id = models.CharField(max_length=44, editable=False, blank=True)
    sheets_template_file_id = models.CharField(max_length=44, blank=True)
    whitelisted_emails = ArrayField(models.EmailField(), editable=False, blank=True)

    def save(self, *args, **kwargs):
        if not self.autogenerated_sheets_folder_id:
            self.autogenerated_sheets_folder_id = self.hunt_folder_id

        if not self.human_files_folder_id:
            self.human_files_folder_id = self.hunt_folder_id

        if not self.whitelisted_emails:
            self.whitelisted_emails = sync_tasks.get_file_user_emails(
                self.hunt_folder_id
            )

        super(GoogleInfo, self).save(*args, **kwargs)
...
class Hunt(models.Model):
    ...
    google_ids = models.OneToOneField(
        GoogleInfo, on_delete=models.CASCADE, null=True, blank=True
    )
    group = models.OneToOneField(
        Group, on_delete=models.CASCADE, editable=False, blank=True, null=True
    )
